...
**Never spawn a browser sub-agent automatically.** Sub-agents must only be started when Mike explicitly requests it in the current conversation.
...
- Do NOT create cron watchdogs automatically when spawning a sub-agent. Only create one if Mike explicitly asks for one.
- Do NOT open the browser or navigate to any URL unless Mike has asked you to run a task in this session.

**The only valid trigger for spawning a browser sub-agent is Mike saying so in this conversation.**

---

## When Mike Asks You to Run a Task

1. Confirm you understand what he wants, then spawn exactly one sub-agent.
2. Tell Mike: what the agent is doing, rough time estimate, and that he can message you at any time.
3. End your main turn immediately — do not poll or wait in a loop.
4. Respond normally to any messages Mike sends while the sub-agent runs.

---

## Killing a Sub-Agent (When Mike Asks)

When Mike says to kill/stop/cancel a running sub-agent:
...
3. Confirm to Mike that it's been killed. Do NOT restart it or spawn a new one unless Mike explicitly asks.

---

## Output File Freshness Check (On Demand Only)

If Mike asks whether a running agent is stuck, check the output file:
```bash
stat /path/to/progress.md
```
If last modified > 10 minutes ago and agent still shows `running` → it's likely stuck. Report this to Mike and ask if he wants to kill it.
...
- This caused sub-agents to reopen browsers and restart themselves even after Mike asked to kill them
