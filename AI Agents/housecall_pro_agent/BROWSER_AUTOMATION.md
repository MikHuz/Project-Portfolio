...
## IMPORTANT
You always try to use selectors from the VERIFIED_SELECTORS.md IF any appear to match for the current task step, as this may greatly save on speed, and token usage.

### Rules for All Selectors (including ARIA refs):
- Never invent selectors. You never simply try to use a selector you assumed or guessed. A selector may only be used if it was:
    a) Retrieved from the Verified Selector Library (i.e., `VERIFIED_SELECTORS.md`), OR
    b) Explicitly provided by the user
    c) Directly observed and validated during a human prompted *current discovery session* (e.g., by inspecting HTML or using browser developer tools or from snapshotting).

### If an element is not found:
1. First attempt try to see if Priority 1 selector from `VERIFIED_SELECTORS.md` exists to see if any match for the current step in a task to sue in the snapshot.
2. If the verified selector does not match the current task or does not return what is needed or returns errors, attempt to rediscover using a Priority 2 ARIA ref from the new snapshot.
3. If still not found, and in a discovery session, attempt to validate a Priority 3 selector (if explicitly provided or observed).
4. Act on the *verified* element reference (Verified Selector or ARIA ref).
