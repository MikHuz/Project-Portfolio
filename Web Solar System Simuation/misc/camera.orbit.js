...
function OrbitCamera(input) {
    ...
    this.update = function (dt, secondsElapsedSinceStart) {
        ...
        if (input.w) {
            var translation = currentForward.clone().multiplyScalar(-this.translationSpeed);
            this.cameraTarget.add(translation);
        }
        if (input.s) {
            var translation = currentForward.clone().multiplyScalar(this.translationSpeed);
            this.cameraTarget.add(translation);
        ...
        if (input.a) {
            var translation = this.getRight().multiplyScalar(-this.translationSpeed);
            this.cameraTarget.add(translation);
        }
        if (input.d) {
            var translation = this.getRight().multiplyScalar(this.translationSpeed);
        ...
        // Update the camera position based on the new target
        this.cameraWorldMatrix.makeTranslation(this.cameraTarget);
    
        // Apply rotation
        let rotate = new Matrix4();
        rotate.makeRotationY(this.yawDegrees);
    ...
    document.onmousemove = function (evt) {
        if (isDragging) {
            this.yawDegrees -= (evt.pageX - lastMouseX) * 0.025;
            this.pitchDegrees -= (evt.pageY - lastMouseY) * 0.025;
    ...
    document.onmousewheel = function (evt) {
        ...
        this.zoomFactor = 1 + this.zoomScale * (this.maxDistance - this.minDistance);
